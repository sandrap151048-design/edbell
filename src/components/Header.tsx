...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 bg-white border-b border-gray-200 shadow-sm ${isScrolled ? 'py-1.5 lg:py-2 shadow-md' : 'py-2 lg:py-2.5'}`}>
        ...
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center group relative z-10 transition-all hover:scale-105">
            ...
            <img 
              ...
              className="h-13 lg:h-16 w-auto object-contain drop-shadow-[0_2px_8px_rgba(37,99,235,0.2)] group-hover:scale-105 transition-transform duration-300"
              onError={(e) => {
            ...
            <div id="logo-fallback-header" className="hidden items-center space-x-2 ml-1">
              <span className={`text-2xl lg:text-3xl font-black tracking-tighter leading-none transition-colors ${isScrolled ? 'text-blue-800' : 'text-white'} group-hover:text-blue-600`}>EdBell</span>
              <span className="text-[9px] lg:text-[10px] font-black text-blue-500 tracking-[0.25em] uppercase opacity-80">edusolutions</span>
            </div>
          </Link>
          ...
          <nav className="hidden xl:flex items-center space-x-0.5">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="relative px-3 xxl:px-5 py-2 text-[10px] xxl:text-[11px] font-black text-gray-700 hover:text-blue-700 transition-all tracking-[0.15em] xxl:tracking-[0.2em] group flex items-center"
              >
                <span className="relative z-10">{item.name}</span>
                <span className="absolute bottom-0 left-3 xxl:left-5 right-3 xxl:right-5 h-0.5 bg-blue-600 scale-x-0 group-hover:scale-x-100 transition-transform origin-left rounded-full"></span>
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            {isAuthenticated ? (
              <div className="hidden sm:flex items-center space-x-2 sm:space-x-4">
                <Link
                  href="/admin"
                  className="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-blue-50 border border-blue-200 rounded-xl text-[9px] sm:text-[10px] font-black tracking-widest text-blue-700 hover:bg-blue-100 transition-all"
                >
                  <User className="h-3 w-3 text-blue-600 outline-none" />
                  <span className="hidden xxl:inline">DASHBOARD</span>
                  <span className="xxl:hidden">ADMIN</span>
                </Link>
                <button
                  onClick={handleLogout}
                  className="p-2 bg-red-50 border border-red-200 rounded-xl text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center outline-none"
                >
            ...
            <button
              ...
              className="xl:hidden p-2.5 sm:p-3 bg-gray-100 border border-gray-200 rounded-2xl text-gray-700 hover:bg-gray-200 transition-all outline-none"
            >
